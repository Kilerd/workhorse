...
function branchSegment(
  ...
  if (
    ...
    branchName.startsWith("task/") &&
    shortName?.startsWith(autoGeneratedPrefix)
  ) {
    const suffix = shortName.slice(autoGeneratedPrefix.length).trim();
    if (suffix) {
      return slugifySegment(suffix, taskId);
    }
  }
